...
class ConcurrencyTestRunner {
  ...
  private async testQueueOperations(): Promise<void> {
    if (!this.concurrency) throw new Error('Concurrency not initialized');
    ...
    if (!priorityQueue) throw new Error('Priority queue not created');
    if (!lifoQueue) throw new Error('LIFO queue not created');
  ...
  private async testTaskExecution(): Promise<void> {
    ...
    let executed = false;
    const taskId = await this.concurrency.submitTask(
      'default',
      'test_task',
      { message: 'Hello World' },
      async (input) => {
    ...
    if (!executed) throw new Error('Task was not executed');
    if (!taskId) throw new Error('Task ID not returned');
  }

  private async testWorkerPoolManagement(): Promise<void> {
    if (!this.concurrency) throw new Error('Concurrency not initialized');
    ...
    if (!pool) throw new Error('Worker pool not created');
    if (pool.size !== 3) throw new Error(`Expected 3 workers, got ${pool.size}`);

    const stats = pool.getStats();
    if (stats.activeWorkers < 0) throw new Error('Invalid worker stats');
  }

  private async testSchedulerBasics(): Promise<void> {
    if (!this.concurrency) throw new Error('Concurrency not initialized');

    let executed = false;
    const futureDate = new Date(Date.now() + 50); // 50ms from now

    const job = this.concurrency.scheduleOnceJob(
      'test_once_job',
      'Test One-Time Job',
      futureDate,
      async (context) => {
    ...
    if (job.id !== 'test_once_job') throw new Error('Job has wrong ID');

    // Wait for job to execute
    await new Promise(resolve => setTimeout(resolve, 100));

    if (!executed) throw new Error('Scheduled job was not executed');
  }
  ...
  private async testCronJobs(): Promise<void> {
    ...
    const job = this.concurrency.scheduleCronJob(
      'cron_test',
      'Cron Test Job',
    ...
    if (!job) throw new Error('Cron job not created');
    if (job.schedule.expression !== '* * * * *') throw new Error('Cron expression not set correctly');
  }

  private async testIntervalJobs(): Promise<void> {
    if (!this.concurrency) throw new Error('Concurrency not initialized');

    let executionCount = 0;

    const job = this.concurrency.scheduleIntervalJob(
      'interval_test',
      'Interval Test Job',
      50, // 50ms interval
      async () => {
    ...
    if (!job) throw new Error('Interval job not created');

    // Wait for a few executions
    await new Promise(resolve => setTimeout(resolve, 150));

    if (executionCount < 1) throw new Error('Interval job did not execute');
  }
  ...
  private async testParallelExecution(): Promise<void> {
    ...
    if (results[0] !== 0) throw new Error('First result should be 0');
    if (results[4] !== 8) throw new Error('Last result should be 8');
  }

  private async testSeriesExecution(): Promise<void> {
    if (!this.concurrency) throw new Error('Concurrency not initialized');
    ...
    if (executionOrder.join(',') !== '0,1,2') throw new Error('Tasks did not execute in series');
  }
  ...
  private async testRetryLogic(): Promise<void> {
    ...
    if (attemptCount !== 3) throw new Error(`Expected 3 attempts, got ${attemptCount}`);
  }
  ...
  private async testTaskPriorities(): Promise<void> {
    ...
    const priorityQueue = this.concurrency.getQueue('priority_queue');
    if (!priorityQueue) throw new Error('Priority queue not found');

    const executionOrder: string[] = [];

    // Submit tasks with different priorities
    await this.concurrency.submitTask(
      'priority_queue',
      'low_priority',
      'low',
      async (input) => {
  ...
  private async testErrorHandling(): Promise<void> {
    ...
    if (!errorCaught) throw new Error('Should have thrown error for nonexistent queue');
    ...
    if (!taskErrorHandled) throw new Error('Task error was not handled properly');
  }
