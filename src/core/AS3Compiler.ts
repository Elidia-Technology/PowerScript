...
import { Timer, PSMath, PSArray, PSVector, PSByteArray } from './AS3Utilities';

export interface AS3CompilerOptions {
    target?: 'es5' | 'es6' | 'es2015' | 'es2017' | 'es2018' | 'es2019' | 'es2020' | 'esnext';
    module?: 'commonjs' | 'amd' | 'system' | 'umd' | 'es6' | 'es2015' | 'esnext';
    strict?: boolean;
    allowDynamicClasses?: boolean;
    enableAS3Utilities?: boolean;
    sourceMaps?: boolean;
    declaration?: boolean;
    outDir?: string;
    rootDir?: string;
...
export interface AS3CompilerResult {
    success: boolean;
    output?: string;
    errors: string[];
    warnings: string[];
...
export class AS3Compiler extends PowerScriptCompiler {
    private options: AS3CompilerOptions;
    private as3Keywords = new Set([
    ...
    constructor(options: AS3CompilerOptions = {}) {
        super();
        this.options = {
            target: 'es2018',
            module: 'commonjs',
     ...
     */
    public async compileAS3(source: string, filename?: string): Promise<AS3CompilerResult> {
        const result: AS3CompilerResult = {
            success: false,
            errors: [],
            warnings: []
        };
     ...
     */
    private preprocessAS3Syntax(source: string): string {
        ...
        processed = processed.replace(
            /var\s+(\w+)\s*:\s*(\w+)(\s*=\s*[^;]+)?;/g,
            (match, varName, varType, initializer) => {
        ...
        processed = processed.replace(
            ...
            (match, className, extendsClause, implementsClause) => {
                let result = `export class ${className}`;
                if (extendsClause) result += extendsClause;
                if (implementsClause) result += implementsClause;
                result += ' {';
                return result;
            }
        );
        ...
        processed = processed.replace(
            /(public|private|protected|internal)\s+(static\s+)?(var|const)\s+(\w+)\s*:\s*(\w+)(\s*=\s*[^;]+)?;/g,
            (match, access, staticMod, varType, varName, type, initializer) => {
        ...
        processed = processed.replace(
            /(public|private|protected|internal)\s+(static\s+)?(override\s+)?function\s+(\w+)\s*\(([^)]*)\)\s*:\s*(\w+)/g,
            (match, access, staticMod, overrideMod, methodName, params, returnType) => {
                const tsAccess = access === 'internal' ? 'public' : access;
                const tsReturnType = this.as3Types.get(returnType) || returnType;
                const processedParams = this.processAS3Parameters(params);
                const static_ = staticMod || '';
        ...
        processed = processed.replace(
            /for\s+each\s*\(\s*var\s+(\w+)\s*:\s*(\w+)\s+in\s+([^)]+)\)/g,
            (match, varName, varType, collection) => {
                const tsType = this.as3Types.get(varType) || varType;
     ...
     */
    private processAS3Parameters(params: string): string {
     ...
     */
    private addAS3Imports(source: string): string {
        const imports = [];
     ...
     */
    private postProcessAS3Output(output: string): string {
...
`;
     ...
     */
    public validateAS3Syntax(source: string): { isValid: boolean; errors: string[]; warnings: string[] } {
        const errors: string[] = [];
     ...
     */
    public getAS3Suggestions(source: string): string[] {
     ...
     */
    public generateAS3TypeDefinitions(): string {
        return `
