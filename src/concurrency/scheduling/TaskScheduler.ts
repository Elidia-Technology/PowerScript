...
export class TaskScheduler extends EventEmitter {
  ...
  private _checkScheduledJobs(): void {
    const now = new Date();
  ...
  private async _executeJob(job: Job, immediate = false): Promise<any> {
    ...
    try {
      ...
      const timeoutPromise = job.options.timeout ? 
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error(`Job ${job.id} timed out after ${job.options.timeout}ms`)), 
            job.options.timeout);
        }) : null;
      ...
      job.metrics.lastError = error as Error;
  ...
  private _calculateNextRun(schedule: Schedule, fromDate?: Date): Date | undefined {
  ...
  private _parseNextCronDate(cronExpression: string, fromDate: Date): Date | undefined {
  ...
  private _updateAverageExecutionTime(job: Job, executionTime: number): void {
