...
export class ExportManager {
     ...
     */
    public async exportDataset(dataset: Dataset, options: ExportOptions): Promise<ExportResult> {
     ...
     */
    public async exportDashboard(dashboard: DashboardLayout, options: ExportOptions): Promise<ExportResult> {
        this.validateExportOptions(options);
     ...
     */
    public async batchExport(items: Array<{
        type: 'dataset' | 'chart' | 'dashboard';
        data: Dataset | ChartConfig | DashboardLayout;
        ...
        for (const item of items) {
            ...
            switch (item.type) {
                case 'dataset':
                    result = await this.exportDataset(item.data as Dataset, item.options);
                    break;
                case 'chart':
                    result = await this.exportChart(item.data as ChartConfig, item.options);
                    break;
                case 'dashboard':
                    result = await this.exportDashboard(item.data as DashboardLayout, item.options);
                    break;
                default:
                    result = {
                        success: false,
                        ...
                        generatedAt: new Date(),
                        error: `Unknown item type: ${(item as any).type}`
                    };
            }
     ...
     * List export history
     */
    public async listExports(): Promise<ExportResult[]> {
     ...
     */
    public async cleanupOldExports(maxAge: number = 30): Promise<number> {
    ...
    private validateExportOptions(options: ExportOptions): void {
        ...
        const supportedFormats: ExportFormat[] = ['json', 'csv', 'xlsx', 'pdf', 'png', 'svg', 'html'];
        if (!supportedFormats.includes(options.format)) {
            throw new Error(`Unsupported export format: ${options.format}`);
        }
    }

    private generateFilename(type: string, name: string, format: ExportFormat): string {
    ...
    private async exportDatasetAsJSON(dataset: Dataset, options: ExportOptions): Promise<Buffer> {
        const exportData = {
            name: dataset.name,
            type: dataset.type,
    ...
    private async exportDatasetAsCSV(dataset: Dataset, options: ExportOptions): Promise<Buffer> {
    ...
    private async exportDatasetAsXLSX(dataset: Dataset, options: ExportOptions): Promise<Buffer> {
    ...
    private async exportChartAsSVG(config: ChartConfig): Promise<Buffer> {
    ...
    private async exportChartAsPNG(config: ChartConfig): Promise<Buffer> {
    ...
    private async exportChartAsHTML(config: ChartConfig): Promise<Buffer> {
...
<body>
    ...
    private async exportDashboardAsHTML(dashboard: DashboardLayout): Promise<Buffer> {
        const html = `
<!DOCTYPE html>
<html>
<head>
...
<body>
                    ...
                    <div class="widget" style="grid-column: span ${widget.position.width}; grid-row: span ${widget.position.height};">
                        ...
                        <div style="margin-top: 10px; padding: 20px; background-color: white; border: 1px dashed #ccc; text-align: center; color: #666;">
                            ${widget.type === 'chart' ? 'Chart Visualization' : 
                              widget.type === 'metric' ? `Metric: ${(widget.config as any)?.label || 'Value'}` :
    ...
    private async exportDashboardAsPDF(dashboard: DashboardLayout): Promise<Buffer> {
