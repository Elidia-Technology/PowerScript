...
export class AnalysisEngine {
     ...
     */
    public async calculateStatistics(dataset: Dataset): Promise<StatisticalSummary> {
     ...
     */
    public async calculateCorrelation(dataset1: Dataset, dataset2: Dataset, method: 'pearson' | 'spearman' = 'pearson'): Promise<number> {
     ...
     */
    public async calculateCorrelationMatrix(datasets: Dataset[], method: 'pearson' | 'spearman' = 'pearson'): Promise<CorrelationMatrix> {
     ...
     */
    public async performRegression(xDataset: Dataset, yDataset: Dataset): Promise<RegressionResult> {
     ...
     */
    public async analyzeTimeSeries(dataset: Dataset): Promise<any> {
        this.validateDataset(dataset);
     ...
     */
    public detectOutliers(dataset: Dataset): DataPoint[] {
        this.validateDataset(dataset);
        ...
        const outliers = dataset.data.filter(point => 
     ...
     */
    public normalizeDataset(dataset: Dataset, method: 'minmax' | 'zscore' = 'minmax'): Dataset {
        this.validateDataset(dataset);
    ...
    private mergeConfig(userConfig?: Partial<AnalysisConfig>): AnalysisConfig {
    ...
    private validateDataset(dataset: Dataset): void {
        ...
        const hasInvalidValues = dataset.data.some(point => 
    ...
    private calculatePValue(tStat: number, df: number): number {
        ...
        if (df <= 0) return 1;
        if (Math.abs(tStat) > 4) return 0.0001;
