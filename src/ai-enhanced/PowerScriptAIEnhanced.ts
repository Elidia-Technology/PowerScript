...
import { HardwareProvider as HWProvider } from './providers/HardwareProvider';

/**
 * PowerScript Enhanced AI/ML - Main orchestration class
 */
export class PowerScriptAIEnhanced extends EventEmitter {
  private config: Required<AIEnhancedConfig>;
  private modelProviders: Map<string, ModelProvider> = new Map();
  private hardwareProviders: Map<DeviceType, HardwareProvider> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new AIEnhancedError('Failed to initialize Enhanced AI', 'INIT_FAILED', { error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  private async initializeHardware(): Promise<void> {
    ...
    const cpuProvider = new HWProvider('cpu');
    ...
    if (this.config.enableGPU) {
      try {
        ...
        if (this.config.cudaPath && fs.existsSync(this.config.cudaPath)) {
          const cudaProvider = new HWProvider('cuda');
        ...
        if (this.config.rocmPath && fs.existsSync(this.config.rocmPath)) {
          const rocmProvider = new HWProvider('rocm');
        ...
        const webgpuProvider = new HWProvider('webgpu');
  ...
  private async initializeModelProviders(): Promise<void> {
    ...
    const localProvider = new LocalModelProvider(this.config.cacheDirectory);
   ...
   */
  private async detectOptimalDevice(): Promise<DeviceType> {
    if (this.config.preferredDevice !== 'auto') {
      return this.config.preferredDevice;
    }

    // Benchmark available devices
    const devices: DeviceType[] = Array.from(this.hardwareProviders.keys());
    let bestDevice: DeviceType = 'cpu';
    let bestScore = 0;
  ...
  async loadModel(
    ...
    if (!this.hardwareProviders.has(targetDevice)) {
      throw new DeviceNotSupportedError(targetDevice, modelId);
    }
   ...
   */
  private async checkMemoryRequirements(modelId: string, device: DeviceType): Promise<void> {
    const provider = this.hardwareProviders.get(device);
    if (!provider) return;
    ...
    if (modelProvider) {
      ...
      if (modelInfo?.memoryRequirement && modelInfo.memoryRequirement > memoryInfo.total - memoryInfo.used) {
        throw new InsufficientMemoryError(
          ...
          memoryInfo.total - memoryInfo.used
        );
      }
    }
  }
  ...
  async generateText(request: TextGenerationRequest): Promise<TextGenerationResponse> {
    ...
    try {
      ...
      if (!model.generateText) {
        throw new AIEnhancedError('Model does not support text generation', 'OPERATION_NOT_SUPPORTED');
      }
  ...
  async summarizeText(
    ...
    if (!model.summarizeText) {
      throw new AIEnhancedError('Model does not support text summarization', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async generateCode(
    ...
    if (!model.generateCode) {
      throw new AIEnhancedError('Model does not support code generation', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async generateImage(request: ImageGenerationRequest): Promise<ImageGenerationResponse> {
    ...
    try {
      ...
      if (!model.generateImage) {
        throw new AIEnhancedError('Model does not support image generation', 'OPERATION_NOT_SUPPORTED');
      }
  ...
  async editImage(
    ...
    if (!model.editImage) {
      throw new AIEnhancedError('Model does not support image editing', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async textToSpeech(text: string, config?: TTSConfig): Promise<Buffer> {
    ...
    if (!model.textToSpeech) {
      throw new AIEnhancedError('Model does not support text-to-speech', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async speechToText(audio: Buffer, config?: STTConfig): Promise<string> {
    ...
    if (!model.speechToText) {
      throw new AIEnhancedError('Model does not support speech-to-text', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async generateAudio(
    ...
    if (!model.generateAudio) {
      throw new AIEnhancedError('Model does not support audio generation', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async generateVideo(request: VideoGenerationRequest): Promise<Buffer> {
    ...
    try {
      ...
      if (!model.generateVideo) {
        throw new AIEnhancedError('Model does not support video generation', 'OPERATION_NOT_SUPPORTED');
      }
  ...
  async generateEmbedding(text: string, modelId?: string): Promise<number[]> {
    ...
    if (!model.generateEmbedding) {
      throw new AIEnhancedError('Model does not support embedding generation', 'OPERATION_NOT_SUPPORTED');
    }
  ...
  async classify(
    ...
    if (!model.classify) {
      throw new AIEnhancedError('Model does not support classification', 'OPERATION_NOT_SUPPORTED');
    }
   ...
   */
  private async createTask(type: AITaskType, input: any): Promise<AITask> {
    const task: AITask = {
   ...
   * List all tasks
   */
  getTasks(status?: AITaskStatus): AITask[] {
  ...
  async cancelTask(taskId: string): Promise<void> {
    ...
    if (!task) {
      throw new AIEnhancedError(`Task not found: ${taskId}`, 'TASK_NOT_FOUND');
    }
  ...
  async getSystemInfo(): Promise<{
    ...
    models: { loaded: ModelInfo[]; available: ModelInfo[] };
    cache: CacheStats;
    ...
    setInterval(() => {
      ...
      for (const [id, task] of Array.from(this.runningTasks.entries())) {
        if (task.status === 'running' && 
          ...
          task.status = 'failed';
          task.error = new TaskTimeoutError(id, this.config.timeoutMs);
...
export default PowerScriptAIEnhanced;