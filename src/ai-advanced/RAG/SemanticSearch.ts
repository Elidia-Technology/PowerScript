...
import { VectorDatabase, VectorSearchResult, VectorSearchOptions } from './VectorDatabase';
...
export class SemanticSearch extends EventEmitter {
  ...
  constructor(embeddingService: EmbeddingService, vectorDatabase: VectorDatabase) {
    super();
   ...
   */
  private async _processQuery(query: string, options?: SearchOptions): Promise<string> {
   ...
   */
  private async _expandQuery(query: string): Promise<QueryExpansion> {
   ...
   */
  private _buildExpandedQuery(originalQuery: string, expansion: QueryExpansion): string {
   ...
   */
  private async _generateQueryEmbedding(query: string): Promise<number[]> {
   ...
   */
  private async _postProcessResults(
   ...
   */
  private _convertToSearchResult(vectorResult: VectorSearchResult): SearchResult {
    return {
      id: vectorResult.id,
   ...
   */
  private _applyDiversityFiltering(results: SearchResult[], threshold: number): SearchResult[] {
   ...
   */
  private async _rerankResults(results: SearchResult[], query: SearchQuery): Promise<SearchResult[]> {
   ...
   */
  private _calculateRelevanceScore(result: SearchResult, query: string): number {
   ...
   */
  private _calculateTextRelevance(text: string, query: string): number {
   ...
   */
  private _calculateMetadataRelevance(metadata: Record<string, any>, query: string): number {
   ...
   */
  private _applyBoostFactors(results: SearchResult[], boostFactors: Record<string, number>): SearchResult[] {
    return results.map(result => {
      let boost = 1.0;
   ...
   */
  private _generateHighlights(results: SearchResult[], query: string): SearchResult[] {
   ...
   */
  private _getSearchStrategy(options?: SearchOptions): string {
    const strategies: string[] = ['vector_similarity'];
    
    if (options?.semanticExpansion) strategies.push('query_expansion');
    if (options?.rerankingEnabled) strategies.push('reranking');
    if (options?.diversityThreshold) strategies.push('diversity_filtering');
    if (options?.boostFactors) strategies.push('boost_factors');
   ...
   */
  private _getCacheKey(query: SearchQuery): string {
   ...
   */
  getCacheStats(): { queryCache: number; expansionCache: number } {
    return {
   ...
   */
  getSuggestedQueries(partialQuery: string, limit = 5): string[] {
   ...
   */
  async findSimilarDocuments(documentId: string, options?: { topK?: number; threshold?: number }): Promise<SearchResult[]> {
    // Get the document
