...
import { PowerScriptNetworkingEnhanced } from '../../networking-enhanced/index';
...
export class MemoryEmbeddingCache implements EmbeddingCache {
  private cache = new Map<string, { embedding: number[]; timestamp: number; ttl?: number }>();
...
export class EmbeddingService extends EventEmitter {
  ...
  private networking: PowerScriptNetworkingEnhanced;
  private cache?: EmbeddingCache;
  private models: Map<string, EmbeddingModel> = new Map();

  constructor(config: EmbeddingServiceConfig, networking?: PowerScriptNetworkingEnhanced) {
    super();
    ...
    this.networking = networking || new PowerScriptNetworkingEnhanced({
   ...
   */
  private _initializeDefaultModels(): void {
    // OpenAI models
   ...
   */
  getModel(name: string): EmbeddingModel | null {
   ...
   */
  addModel(model: EmbeddingModel): void {
  ...
  async embedBatch(request: EmbeddingBatchRequest): Promise<EmbeddingBatchResponse> {
    ...
    const response: EmbeddingBatchResponse = {
      embeddings,
   ...
   */
  private async _generateOpenAIEmbedding(text: string, model: EmbeddingModel): Promise<{ embedding: number[]; tokensUsed: number }> {
    if (!model.apiKey) {
   ...
   */
  private async _generateHuggingFaceEmbedding(text: string, model: EmbeddingModel): Promise<number[]> {
   ...
   */
  private async _generateLocalEmbedding(text: string, model: EmbeddingModel): Promise<number[]> {
   ...
   */
  private async _generateCustomEmbedding(text: string, model: EmbeddingModel): Promise<number[]> {
    if (!model.endpoint) {
   ...
   */
  private _getCacheKey(text: string, model: string): string {
