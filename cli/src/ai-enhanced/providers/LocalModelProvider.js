 ...
 */
class MockLoadedModel extends events_1.EventEmitter {
    ...
    async generateCode(prompt, config) {
        ...
        return templates[config.language] ||
 ...
 */
class LocalModelProvider extends events_1.EventEmitter {
    ...
    supportsDevice(device) {
        const supportedDevices = ['cpu', 'cuda', 'rocm', 'mps', 'webgpu'];
    ...
    async loadModel(modelId, device = 'cpu') {
        ...
        if (!model.supportedDevices.includes(device)) {
            throw new types_1.AIEnhancedError(`Model ${modelId} does not support device ${device}`, 'DEVICE_NOT_SUPPORTED', { modelId, device, supportedDevices: model.supportedDevices });
        }
        const modelPath = path.join(this.cacheDir, `${modelId}.bin`);
...
exports.default = LocalModelProvider;
