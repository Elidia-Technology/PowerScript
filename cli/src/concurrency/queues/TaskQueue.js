...
class TaskQueue extends events_1.EventEmitter {
    constructor(id, type = types_1.QueueType.FIFO, options = {}, name) {
        super();
        this.tasks = [];
        this._running = false;
        ...
        this.options = {
            ...
            defaultPriority: options.defaultPriority ?? types_1.TaskPriority.NORMAL,
    ...
    async enqueue(type, input, handler, options = {}) {
        if (this.tasks.length >= this.options.maxSize) {
            throw new types_1.QueueError(`Queue ${this.id} is full (max size: ${this.options.maxSize})`, this.id);
        }
        ...
        this._emitEvent(types_1.EventType.TASK_STARTED, { taskId, type, queueId: this.id });
     ...
     */
    clear() {
        const pendingTasks = this.tasks.filter(task => task.status === types_1.TaskStatus.PENDING);
        this.tasks = this.tasks.filter(task => task.status !== types_1.TaskStatus.PENDING);
        this.metrics.currentSize = this.tasks.length;
        pendingTasks.forEach(task => {
            task.status = types_1.TaskStatus.CANCELLED;
            this._emitTaskEvent(task, types_1.EventType.TASK_FAILED, { reason: 'Queue cleared' });
        });
    }
    /**
     * Get task by ID
     ...
     */
    getStats() {
        const pendingTasks = this.tasks.filter(task => task.status === types_1.TaskStatus.PENDING).length;
        return {
            ...this.metrics,
    ...
    _insertTask(task) {
        switch (this.type) {
            case types_1.QueueType.FIFO:
                this.tasks.push(task);
                break;
            case types_1.QueueType.LIFO:
                this.tasks.unshift(task);
                break;
            case types_1.QueueType.PRIORITY:
                this._insertByPriority(task);
                break;
            case types_1.QueueType.DELAYED:
    ...
    async _processNext() {
        ...
        this._emitTaskEvent(task, types_1.EventType.TASK_STARTED);
        try {
    ...
    _getNextTask() {
        return this.tasks.find(task => task.status === types_1.TaskStatus.PENDING &&
    ...
    async _handleTaskSuccess(task, result) {
        ...
        this._emitTaskEvent(task, types_1.EventType.TASK_COMPLETED, { result });
    ...
    async _handleTaskError(task, error) {
        ...
        this._emitTaskEvent(task, types_1.EventType.TASK_FAILED, { error: error.message });
    ...
    _setupEventHandlers() {
        this.on('taskCompleted', () => {
            if (this.metrics.currentSize === 0 && this._concurrentCount === 0) {
                this._emitEvent(types_1.EventType.QUEUE_EMPTY, { queueId: this.id });
            }
        });
        this.on('taskFailed', () => {
            if (this.metrics.currentSize === 0 && this._concurrentCount === 0) {
