...
class TaskScheduler extends events_1.EventEmitter {
    ...
    start() {
        ...
        this._emitEvent(types_1.EventType.JOB_SCHEDULED, { message: 'Scheduler started' });
    }
    /**
     * Stop the scheduler
     */
    stop() {
        ...
        this._emitEvent(types_1.EventType.JOB_SCHEDULED, { message: 'Scheduler stopped' });
    }
    /**
     * Schedule a new job
     */
    scheduleJob(id, name, schedule, handler, options = {}) {
        if (this.jobs.has(id)) {
            throw new types_1.SchedulerError(`Job with id '${id}' already exists`, id);
        }
        const job = {
            id,
            name,
            ...
            status: options.enabled ? types_1.JobStatus.SCHEDULED : types_1.JobStatus.DISABLED,
            metrics: {
                totalExecutions: 0,
        ...
        this._emitEvent(types_1.EventType.JOB_SCHEDULED, { jobId: id, nextRun: job.nextRun });
        return job;
    }
    /**
     * Schedule a cron job
    ...
    scheduleCronJob(id, name, cronExpression, handler, options = {}) {
        const schedule = {
            type: types_1.ScheduleType.CRON,
    ...
    scheduleIntervalJob(id, name, intervalMs, handler, options = {}) {
        const schedule = {
            type: types_1.ScheduleType.INTERVAL,
            interval: intervalMs
        };
        return this.scheduleJob(id, name, schedule, handler, options);
    }
    ...
    scheduleOnceJob(id, name, runAt, handler, options = {}) {
        const schedule = {
            type: types_1.ScheduleType.ONCE,
            times: [runAt]
        };
        return this.scheduleJob(id, name, schedule, handler, options);
    }
    ...
    cancelJob(jobId) {
        ...
        this._emitEvent(types_1.EventType.JOB_SCHEDULED, { jobId, status: 'cancelled' });
        return true;
    }
    /**
     * Enable a job
    ...
    enableJob(jobId) {
        ...
        if (!job)
            return false;
        job.status = types_1.JobStatus.SCHEDULED;
    ...
    disableJob(jobId) {
        ...
        if (!job)
            return false;
        job.status = types_1.JobStatus.DISABLED;
    ...
    async executeJobNow(jobId) {
        const job = this.jobs.get(jobId);
        if (!job) {
            throw new types_1.SchedulerError(`Job '${jobId}' not found`, jobId);
        }
    ...
    getStats() {
        ...
        return {
            ...
            scheduledJobs: jobs.filter(j => j.status === types_1.JobStatus.SCHEDULED).length,
            runningJobs: jobs.filter(j => j.status === types_1.JobStatus.RUNNING).length,
            disabledJobs: jobs.filter(j => j.status === types_1.JobStatus.DISABLED).length,
            isRunning: this.isRunning
        };
    }
    _checkScheduledJobs() {
        const now = new Date();
        const runningJobs = Array.from(this.jobs.values()).filter(j => j.status === types_1.JobStatus.RUNNING).length;
        ...
        for (const job of this.jobs.values()) {
            if (job.status === types_1.JobStatus.SCHEDULED &&
                ...
                if (job.schedule.type !== types_1.ScheduleType.ONCE) {
    ...
    async _executeJob(job, immediate = false) {
        ...
        this._emitEvent(types_1.EventType.JOB_EXECUTED, {
            jobId: job.id,
            executionId,
            startTime,
            immediate
        ...
        try {
            ...
            job.status = job.schedule.type === types_1.ScheduleType.ONCE ? types_1.JobStatus.COMPLETED : types_1.JobStatus.SCHEDULED;
            ...
            job.status = job.schedule.type === types_1.ScheduleType.ONCE ? types_1.JobStatus.FAILED : types_1.JobStatus.SCHEDULED;
            this._updateAverageExecutionTime(job, executionTime);
            this._emitEvent(types_1.EventType.JOB_EXECUTED, {
                jobId: job.id,
                executionId,
