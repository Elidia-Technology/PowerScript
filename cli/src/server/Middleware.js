...
class Middleware {
    ...
    static cors(options = {}) {
        return async (req, res, next) => {
            const { origin = '*', methods = ['GET', 'HEAD', 'PUT', 'PATCH', 'POST', 'DELETE'], allowedHeaders = ['Content-Type', 'Authorization'], credentials = false, maxAge = 86400 } = options;
            // Set CORS headers
    ...
    static rateLimit(config) {
        ...
        max = 100, message = 'Too many requests, please try again later', standardHeaders = true, legacyHeaders = false } = config;
        return async (req, res, next) => {
    ...
    static helmet(options = {}) {
        return async (req, res, next) => {
            const { contentSecurityPolicy = true, crossOriginEmbedderPolicy = true, crossOriginOpenerPolicy = true, crossOriginResourcePolicy = true, dnsPrefetchControl = true, frameguard = { action: 'sameorigin' }, hidePoweredBy = true, hsts = true, ieNoOpen = true, noSniff = true, originAgentCluster = true, permittedCrossDomainPolicies = false, referrerPolicy = { policy: 'no-referrer' }, xssFilter = true } = options;
...
Middleware._logger = new Logger_1.Logger({ level: 'info', outputs: [new Logger_1.ConsoleLogOutput()] });
Middleware._rateLimitStore = new Map();
