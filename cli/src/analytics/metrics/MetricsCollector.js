...
class MetricsCollector {
    ...
    getMetricHistory(metricName, timeRange) {
        ...
        if (timeRange) {
            filteredData = metricData.filter(metric => metric.timestamp >= timeRange.start && metric.timestamp <= timeRange.end);
        }
        return {
            metric: metricName,
     ...
     */
    getAggregatedMetrics(metricName, aggregation, interval = '1m', timeRange) {
     ...
     */
    getMetricStatistics(metricName, timeRange) {
        const history = this.getMetricHistory(metricName, timeRange);
